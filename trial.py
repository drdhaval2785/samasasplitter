...
def permut(word,lstrep,dictionary,allowed):
	...
	for allow in allowed:
		entr = [entr for (a,(b,entr)) in subs if a==len(allow)-1]
		print allow[:-1], len(allow), entr
if __name__=="__main__":
	print datetime.datetime.now()
	inputword = sys.argv[1]
	allowed = st1(inputword,dictionary)
	print permut(inputword,lstrep1,dictionary,allowed)
	print datetime.datetime.now()
